...
def generate_style(base_image_of_person_url: str, item_image_urls: list[str], item_category_list: list[str], client=None) -> Image.Image:
...
class ChatChain:
    def __init__(self, model, client, system_prompt, enable_image_generation=False, function_registry=None):
        self.model = model
        self.client = client
    ...
    def chat(self, message: Union[str, List], images: Optional[List] = None, generate_image=None):
        ...
        if tool_calls:
            # Create function registry with available functions and pass client for generate_style
            function_registry = self.function_registry.copy()
            if 'generate_style' in function_registry:
                # Inject client into generate_style function
                def generate_style_with_client(**kwargs):
                    return function_registry['generate_style'](client=self.client, **kwargs)
                function_registry['generate_style'] = generate_style_with_client
