...
async def generate_style_endpoint(request: StyleGenerationRequest):
    ...
    try:
        ...
        all_image_urls = [request.base_image_url] + request.item_image_urls
        image_parts = []
        ...
        for image_url in all_image_urls:
            ...
            # Convert to PIL image and then to bytes for Gemini
            pil_image = Image.open(BytesIO(response.content))
            img_bytes, mime_type = pil_image_to_bytes(pil_image)
        ...
        for i, category in enumerate(request.item_categories):
